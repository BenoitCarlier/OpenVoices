...
def featureAndTrain(list_of_dirs, mt_win, mt_step, st_win, st_step, 
    ...
    if classifier_type == "svm" or classifier_type == "svm_rbf":
        ...
        classifier_par = numpy.array([7.0, 8.0, 9.0, 10.0, 11.0, 12.0])
    ...
    bestParam = evaluateclassifier(features, classNames, 100, classifier_type, classifier_par, 0, perTrain, model_name)
...
def evaluateclassifier(features, class_names, n_exp, classifier_name, Params, parameterMode, perTrain=0.90, model_name='more_information.pkl'):
    '''
    ARGUMENTS:
        features:     a list ([numOfClasses x 1]) whose elements containt numpy matrices of features.
                each matrix features[i] of class i is [n_samples x numOfDimensions]
        ...
        model_name (default: more_information): in order to dump in a file :
            Params
            ac_all
            f1_all
...
def fileClassification(inputFile, model_name, model_type):
    ...
    if isinstance(x, int):                                 # audio file IO problem
        print("audio file IO problem:  audio int instance")
        return (-1, -1, -1)
    if x.shape[0] / float(Fs) <= mt_win:
        print("audio file IO problem:  x.shape[0] / float(Fs) <= mt_win")
        return (-1, -1, -1)
